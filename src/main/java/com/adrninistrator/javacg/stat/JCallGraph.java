    ...
    public boolean run(JavaCGConfigureWrapper javaCGConfigureWrapper) {
        ...
        try (Writer classAnnotationWriter = JavaCGFileUtil.genBufferedWriter(javaCGOutputInfo.getMainFilePath(JavaCGOutPutFileTypeEnum.OPFTE_CLASS_ANNOTATION));
            ...
            if (!handleJar(newJarFilePath, methodCallWriter, springBeanWriter) || failCounter.getCount() > 0) {
             ...
             classAnnotationWriter.flush();
             classInfoWriter.flush();
             classNameWriter.flush();
             classSignatureEI1Writer.flush();
             extendsImplWriter.flush();
             fieldAnnotationWriter.flush();
             fieldInfoWriter.flush();
             fieldGenericsTypeWriter.flush();
             fieldRelationshipWriter.flush();
             getMethodWriter.flush();
             innerClassWriter.flush();
             jarInfoWriter.flush();
             lambdaMethodInfoWriter.flush();
             methodAnnotationWriter.flush();
             methodArgumentWriter.flush();
             methodArgAnnotationWriter.flush();
             methodArgGenericsTypeWriter.flush();
             methodCallInfoWriter.flush();
             methodCallMethodCallReturnWriter.flush();
             methodCallStaticFieldWriter.flush();
             methodReturnArgSeqWriter.flush();
             methodReturnCallIdWriter.flush();
             methodCallWriter.flush();
             methodInfoWriter.flush();
             methodLineNumberWriter.flush();
             methodReturnGenericsTypeWriter.flush();
             methodCatchWriter.flush();
             methodFinallyWriter.flush();
             methodThrowWriter.flush();
             setMethodWriter.flush();
             springBeanWriter.flush();
             staticFinalFieldMethodCallIdWriter.flush();
             logMethodSpendTimeWriter.flush();
